...
const CreditCardItem = ({ card, onSetDefault, onRemove }) => {
  ...
  return (
    <div className={`border rounded-lg bg-white shadow-sm overflow-hidden ${card.isDefault ? 'border-blue-300 ring-1 ring-blue-100' : 'border-slate-200'}`}>
      <div className="p-5 flex items-start gap-4">
        <div className={`w-14 h-9 rounded-md ${brandColor} flex items-center justify-center text-white text-[11px] font-bold tracking-wider shrink-0`}>
      ...
      <div className="border-t border-slate-100 px-5 py-3 flex items-center justify-between">
        {!card.isDefault ? (
          ...
          <span className="text-xs text-slate-400">Primary payment method</span>
        )}
        <button
