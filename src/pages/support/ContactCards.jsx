...
const ContactCards = () => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
    {cards.map(({ icon: Icon, title, desc, action, bg, color }) => (
      <div key={title} className="border border-slate-200 rounded-xl bg-white p-6 text-center shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200">
        ...
        <a href={action.href} className="text-sm text-blue-600 font-medium hover:underline inline-block">
          {action.label}
        </a>
      </div>
    ))}
