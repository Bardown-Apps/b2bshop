...
const HeroSection = ({ onSignIn }) => (
  <section className="relative bg-slate-950 text-white overflow-hidden min-h-[400px] sm:min-h-[480px] md:min-h-[580px] flex items-center">
    ...
    <div className="absolute bottom-0 left-1/4 w-80 h-80 bg-blue-900/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '3s' }} />
    ...
    <div className="relative max-w-[1400px] mx-auto px-4 md:px-8 py-16 md:py-24 flex flex-col md:flex-row items-center gap-10 md:gap-16 w-full">
      <div className="flex-1 text-center md:text-left">
        <div
          className="inline-block bg-red-600 text-white text-[10px] sm:text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-5 md:mb-7 animate-fade-up"
        >
        ...
        >
          More Flex.<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-600">Less Wait.</span>
        </h1>
        <p
      ...
      <div className="flex flex-row md:flex-col gap-4 md:gap-5 shrink-0">
        {HERO_STATS.map((s, i) => (
          ...
          >
            <p className="text-2xl sm:text-4xl font-black text-red-400">{s.num}</p>
            <p className="text-[10px] sm:text-xs text-slate-500 font-medium uppercase tracking-wide mt-1">{s.label}</p>
          </div>
        ))}
      </div>
    </div>
